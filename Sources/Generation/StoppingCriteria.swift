...
public protocol StoppingCriteria: Sendable {
    ...
    /// - Parameters:
    ///   - tokens: the generated tokens so far. When a streaming callback is active this
    ///     is the full sequence (prompt + completion); otherwise — to avoid an O(n)
    ///     per-token GPU→CPU sync — it is just the most recently generated token.
    ///     Last-token criteria (e.g. `EosTokenCriteria`) work in both cases; criteria that
    ///     need more history should supply a streaming callback or track state internally.
